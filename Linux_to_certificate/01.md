...
+ [15. Работа с дисками](#15-работа-с-дисками)
  + [15.1 Чтение информации](#151-чтение-информации)
  + [15.2 Монтирование диска](#152-монтирование-диска)
  + [15.3 LVM (logical volume management)](#153-lvm-logical-volume-management)
+ [16. Docker](#16-docker)
  + [16.1 Введение, установка](#161-введение-установка)
  + [16.2 Список команд, первый контейнер](#162-список-команд-первый-контейнер)
  + [16.3 Контейнер с документацией](#163-контейнер-с-документацией)
  + [16.4 Веб-сервер Nginx в Docker](#164-веб-сервер-nginx-в-docker)
  + [16.6 Docker Compose](#166-docker-compose)
  + [16.7 Удаление ресурсов](#167-удаление-ресурсов)
...
### 15.1 Чтение информации

[Video](/Linux_to_certificate/videos/15_01_Работа_с_дисками_Чтение_информации_Stepik.mp4)

**Список изученных команд:**

+ `ls /dev` - вывод содержимого папки `/dev`
+ `ls /dev/sd*` - вывод отфильтрованного содержимого папки `/dev`

+ `whatis df` - вывод однострочной справки по команде `df`
+ `df -h` - вывод информации об использовании дискового пространства файловых систем
+ `df -hT` - добавление колонки **Type** (тип устройства) в предыдущую команду
+ `df -hT -t ext4` - фильтрация по колонке **Type** в предыдущей команде
+ `df -hT -t ext4 -i` - вывод информации об используемом месте в `inodes`

+ `whatis du` - вывод однострочной справки по команде `du`
+ `du ~ -s` - подсчет занимаемого места домашним каталогом пользователя (`~`)
+ `du ~ -sh` - подсчет занимаемого места домашним каталогом пользователя (`~`)
+ `du -hc` - подсчет занимаемого места текущей папкой
+ `du *.txt -h` - подсчет занимаемого места файлами с расширением 'txt'

+ `whatis mount` - вывод однострочной справки по команде `mount`
+ `mount` - вывод информации командой `mount` без параметров

+ `whatis fdisk` - вывод однострочной справки по команде `fdisk`
+ `whatis gdisk` - вывод однострочной справки по команде `gdisk`
+ `sudo fdisk -l /dev/sda` - вывод информации о диске `/dev/sda`

+ `whatis lsblk` - вывод однострочной справки по команде `lsblk`
...
### 15.2 Монтирование диска

[Video](/Linux_to_certificate/videos/15_02_Работа_с_дисками_Монтирование_диска_Stepik.mp4)
...
[Video](/Linux_to_certificate/videos/15_03_Работа_с_дисками_Монтирование_диска_Stepik.mp4)
...
### 15.3 LVM (logical volume management)

[Video](/Linux_to_certificate/videos/15_04_LVM_logical_volume_management_Stepik.mp4)

**Проблемы, решаемые LVM:**

+ Динамическое выделение места на диске
+ Изменение размера томов без отключения
+ Консистентные бэкапы (snapshot)
+ Кэширование данных
...
+ `sudo apt install lvm2 -y` - установить пакет **lvm2**
...
![Компоненты LVM](./img/15_01.png)

[Video](/Linux_to_certificate/videos/15_05_LVM_logical_volume_management_Stepik.mp4)
...
[Video](/Linux_to_certificate/videos/15_06_LVM_logical_volume_management_Stepik.mp4)
...
[Video](/Linux_to_certificate/videos/15_07_LVM_logical_volume_management_Stepik.mp4)
...
**Таблица основных команд инфраструктуры LVM:**

![Таблица основных команд инфраструктуры LVM](./img/15_02.png)
...
### 16.1 Введение, установка

[Video](/Linux_to_certificate/videos/16_01_Docker_Введение_установка_Stepik.mp4)

**Список команд:**

```sh
# вызов справки (проверка: установлен ли пакет)
docker --help

# установка пакета docker.io
sudo apt install docker.io -y

# вызов однострочной справки по Docker
whatis docker
```

**Предпосылки появления Docker, как инструмента для контейнеризации:**

![Предпосылки появления Docker, как инструмента для контейнеризации](./img/16_01.png)

---
[Содержание](#содержание)

### 16.2 Список команд, первый контейнер

[Video](/Linux_to_certificate/videos/16_02_Docker_Список_команд_первый_контейнер_Stepik.mp4)
...
search  # поиск образа в Docker Hub
pull    # скачать образ из Docker Hub
rm      # удалить контейнер
rmi     # удалить образ
stop    # остановить контейнер
build   # собрать образ по инструкциям в Dockerfile
exec    # исполнить команду внутри контейнера
```

**Компоненты Docker:**

![Компоненты Docker](./img/16_02.png)

---
[Содержание](#содержание)

### 16.3 Контейнер с документацией

[Video](/Linux_to_certificate/videos/16_03_Docker_Контейнер_с_документацией_Stepik.mp4)
...
# вывести последние 10 строк из файла со списко групп
tail /etc/group
...
# то же, но без прав root-пользователя
...
## 16.4 Веб-сервер Nginx в Docker

```sh
Список команд:

# поиск контейнера с названием nginx
docker search nginx

# вывод списка локальных образов 
docker images

# скачивание образа с именем nginx
docker pull nginx

# запуск контейнера из образа nginx с указанием проброшенного порта и в режиме detach
...
# проверка файлов конфигурации веб-сервера Nginx
nginx -t

# проверка статуса службы Nginx
systemctl status nginx

# вывод справки Docker
docker --help

# запуск терминала bash внутри контейнера
...
# перезапись содержимого файла (ввод с консоли)
cat > <filename>
```
...
[Video](/Linux_to_certificate/videos/16_05_Dockerfile_Stepik.mp4)

**Полезные ссылки:**

[Документация по Dockerfile](https://docs.docker.com/reference/dockerfile/)

**Список команд:**

```sh
# вызов однострочной справки по Dockerfile
whatis dockerfile

# создадим папку для проекта
mkdir nginx_project

# переместимсмяв созданную папку
cd nginx_project
...
**Порядок работы с Dockerfile:**

![Порядок работы с Dockerfile](./img/16_03.png)

[Video](/Linux_to_certificate/videos/16_06_Dockerfile_Stepik.mp4)

**Список команд:**

```sh
# выводим на экран содержимое Dockerfile
cat Dockerfile 

# выводим список локальных образов
docker images

# запускаем сборку образа из инструкций в Dockerfile
...
**Написанные инструкции в Dockerfile:**
...
[Video](/Linux_to_certificate/videos/16_07_Docker_Compose_Stepik.mp4)

**Полезные ссылки:**

+ [Примеры типовых конфигураций](https://github.com/docker/awesome-compose/tree/master)
+ [Официальна документация:](https://docs.docker.com/compose/)
+ [Статья на тему](https://habr.com/ru/companies/ruvds/articles/450312/)
...
# вызываем краткую справку по docker-compose
docker-compose --help
...
### 16.7 Удаление ресурсов

[Video](/Linux_to_certificate/videos/16_08_Docker_Удаление_ресурсов_Stepik.mp4)

**Полезные ссылки:**

+ [Официальная документация (команда rm)](https://docs.docker.com/reference/cli/docker/container/rm/)
+ [Статья на тему урока](https://timeweb.cloud/tutorials/docker/kak-udalit-obrazy-kontejnery-i-toma-docker)
...
# выведем список всей истории запущенных контейнеров (в том числе остановленных)
docker ps -a

# удалим контейнер по его идентификатору
docker rm 7b6eef994cd5

# остановим контейнер по его идентификатору
docker stop b2d5a0d52d59

# форсированно удалим контейнер (даже если он сейчас запущен)
docker rm f3ef5dee0e74 -f

# выведем список идентификаторов контейнеров
docker ps -aq

# очистим всю историю запущенных контейнеров
docker rm $(docker ps -aq)

# выведем список образов
docker images

# удалим образ по его имени
docker rmi hello-world

# удалим образ по его идентификатору
